...
var SqueezeBox = {
	...
	showContent: function() {
		...
		this.fx.content.onComplete = function(){
		popupCall(this);
		
		/*$('citeform').addEvent('submit', function(e) {
	var log = $('sbox-content').addClass('sbox-loading');
	});

		
		$('link').focus();
		$('link').addEvents({
	'keyup': function(event) {if($('link').value.toLowerCase().indexOf("http://") != 0 && $('link').value.length >= 7) {$('link').value = "http://"+$('link').value;}
	$('linktext').innerText = $('link').value;$('linktext').href = $('link').value;}
	}); */
